// i18n translations for Minopex website
// Supports: English (en), Arabic (ar), Greek (el)

export type Language = 'en' | 'ar' | 'el' | 'es';

export const translations = {
  en: {
    nav: {
      home: 'Home',
      about: 'About',
      whatWeDo: 'What We Do',
      sustainability: 'Sustainability',
      careers: 'Careers',
      insights: 'Insights',
      contact: 'Contact',
      suppliers: 'Suppliers',
    },
    footer: {
      offices: 'Offices',
      legal: 'Legal',
      privacy: 'Privacy Policy',
      paia: 'PAIA',
      sitemap: 'Sitemap',
      followUs: 'Follow Us',
      copyright: '© 2026 Minopex. All rights reserved. A DRA Global company.',
    },
    home: {
      tagline: 'Leading global plant operations and maintenance specialists',
      heroHeading: 'Operational Excellence in Global Mining',
      heroSubheading: 'Delivering innovative solutions for mining operations across 14+ countries',
      learnMore: 'Learn More',
      surfaceOperations: 'Surface Operations',
      surfaceDesc: 'Processing plants, labs, and water management',
      undergroundOperations: 'Underground Operations',
      undergroundDesc: 'Mechanised mining with operational readiness',
      seeWhatWeDo: 'See What We Do',
      stats: {
        employees: 'Employees',
        countries: 'Countries',
        plants: 'Plants Operated',
        ore: 'Ore/Month',
      },
      ourPeople: 'Our Biggest Asset is Our People',
      ourPeopleDesc: 'With 2,000+ skilled professionals across 14+ countries, our team is the foundation of our success. We invest in safety, development, and creating a workplace where excellence thrives.',
      partnership: 'Part of DRA Global',
      partnershipDesc: 'Minopex is a leading company within the DRA Global group, delivering world-class mining operations and maintenance solutions.',
      ctaDesc: 'Ready to partner with Minopex? Get in touch with our team today.',
    },
    about: {
      heading: 'About Minopex',
      subheading: 'Established in 1996, Minopex has grown to become a leading global mining operations specialist',
      history: 'Our History',
      historyDesc: 'Founded in 1996 with a commitment to operational excellence, Minopex has evolved into a trusted partner for mining operations worldwide. With 30+ years of combined experience, we have successfully operated 50+ plants across multiple commodities.',
      globalFootprint: 'Global Footprint',
      commodities: 'Commodities We Serve',
      coal: 'Coal',
      chrome: 'Chrome',
      ironOre: 'Iron Ore',
      gold: 'Gold',
      diamonds: 'Diamonds',
      pgm: 'PGM',
      rareEarths: 'Rare Earths',
      lithium: 'Lithium',
      batteryMinerals: 'Battery Minerals',
      criticalMinerals: 'Critical Minerals',
      values: 'Our Values',
      focus: 'Focus',
      focusDesc: 'Concentrated expertise on operational excellence',
      accountability: 'Accountability',
      accountabilityDesc: 'Transparent and responsible operations',
      safety: 'Safety',
      safetyDesc: 'Zero harm to people and environment',
      technology: 'Technology',
      technologyDesc: 'Innovation driving operational efficiency',
      integrity: 'Integrity',
      integrityDesc: 'Ethical practices in all operations',
      respect: 'Respect',
      respectDesc: 'Valuing people and communities',
      ctaDesc: 'Discover what we do and how we can help your mining operations.',
    },
    whatWeDo: {
      heading: 'What We Do',
      subheading: 'Comprehensive mining operations and maintenance solutions',
      surfaceOpsHeading: 'Surface Operations',
      surfaceOpsDesc: 'From processing plants to laboratory services, we manage the complete surface operations lifecycle.',
      undergroundOpsHeading: 'Underground Operations',
      undergroundOpsDesc: 'Advanced mechanised mining with safety-first approach and operational readiness.',
      surface: {
        plants: 'Processing plants & facilities',
        lab: 'Laboratory services & quality assurance',
        water: 'Water management systems',
      },
      underground: {
        equipment: 'Advanced mechanised mining equipment',
        safety: 'Safety-first operational protocols',
        readiness: 'Operational readiness & optimization',
      },
      coreServices: 'Our Core Services',
      services: {
        engineer: 'Engineer',
        engineerDesc: 'Design and optimize mining operations',
        optimise: 'Optimise',
        optimiseDesc: 'Maximize efficiency and productivity',
        innovate: 'Innovate',
        innovateDesc: 'Implement cutting-edge technologies',
        maintain: 'Maintain',
        maintainDesc: 'Ensure reliable, safe operations',
      },
      qls: 'Quality Laboratory Services',
      qlsDesc: 'State-of-the-art laboratory services for mineral analysis and quality assurance',
    },
    sustainability: {
      heading: 'Sustainability & Responsibility',
      subheading: 'Mining responsibly for a sustainable future',
      esd: 'Environmental Stewardship & Development',
      esdDesc: 'We are committed to protecting ecosystems and supporting community development through responsible mining practices.',
      csi: 'Community Social Investment',
      csiDesc: 'Building strong relationships with local communities through education, healthcare, and economic development initiatives.',
      environmental: 'Environmental Management',
      water: 'Water Management',
      waterDesc: 'Advanced treatment and recycling systems',
      land: 'Land Rehabilitation',
      landDesc: 'Progressive restoration of mining sites',
      air: 'Air Quality Control',
      airDesc: 'Monitoring and dust control systems',
      waste: 'Waste Management',
      wasteDesc: 'Responsible recycling and disposal',
      ourCommitment: 'Our Commitment',
      commitmentDesc: 'We are committed to responsible mining practices that protect the environment, support communities, and create lasting value for future generations.',
      commitmentZeroHarm: 'Zero harm to people and environment',
      commitmentImpact: 'Net positive impact on communities',
      commitmentTransparent: 'Transparent and accountable operations',
      ctaHeading: 'Join Us in Building a Sustainable Future',
      ctaDesc: 'Partner with Minopex to achieve your mining goals responsibly.',
    },
    careers: {
      heading: 'Careers at Minopex',
      subheading: 'People are our greatest asset',
      whyJoin: 'Why Join Minopex?',
      whyJoinDesc: 'We offer competitive compensation, professional development, and a workplace culture that values safety, integrity, and innovation.',
      whyChoose: 'Why Choose Minopex',
      applyNow: 'Apply Now',
      equalOpportunity: 'Minopex is an Equal Opportunity Employer. We celebrate diversity and are committed to creating an inclusive environment.',
      equalOpportunityHeading: 'Equal Opportunity Employer',
      ctaHeading: 'Ready to Join Our Team?',
      ctaDesc: 'Explore current opportunities and start your career with Minopex.',
      benefits: {
        devTitle: 'Professional Development',
        devDesc: 'Continuous learning and career growth opportunities',
        cultureTitle: 'Collaborative Culture',
        cultureDesc: 'Work with talented professionals globally',
        compTitle: 'Competitive Compensation',
        compDesc: 'Attractive salary and benefits packages',
        globalTitle: 'Global Opportunities',
        globalDesc: 'Work across 14+ countries worldwide',
      },
    },
    contact: {
      heading: 'Contact Us',
      subheading: 'Get in touch with our team',
      offices: 'Our Offices',
      johannesburg: 'Johannesburg',
      lesotho: 'Lesotho',
      mozambique: 'Mozambique',
      saudiArabia: 'Saudi Arabia',
      sendMessage: 'Send us a Message',
      phone: 'Phone',
      email: 'Email',
      headquarters: 'Headquarters',
      form: {
        name: 'Full Name',
        email: 'Email Address',
        phone: 'Phone Number',
        subject: 'Subject',
        message: 'Message',
        submit: 'Send Message',
        success: 'Thank you for your message. We will get back to you soon.',
      },
    },
    leadership: {
      heading: 'Leadership',
      subheading: 'Meet our leadership team',
      comingSoon: 'Leadership team information coming soon.',
    },
    insights: {
      heading: 'Insights & Blog',
      subheading: 'Latest news and insights from Minopex',
      comingSoon: 'Blog and insights coming soon.',
    },
    suppliers: {
      heading: 'Suppliers',
      subheading: 'Information for our suppliers',
      comingSoon: 'Supplier information and resources coming soon.',
    },
    privacy: {
      heading: 'Privacy Policy',
      intro: 'Minopex is committed to protecting your privacy. This privacy policy explains how we collect, use, and protect your personal information.',
      comingSoon: 'Full privacy policy content coming soon.',
    },
    paia: {
      heading: 'PAIA',
      subheading: 'Promotion of Access to Information Act',
      manualHeading: 'PAIA Manual',
      intro: 'In accordance with the Promotion of Access to Information Act (PAIA), Minopex provides access to information held by the company.',
      comingSoon: 'PAIA manual and information requests coming soon.',
    },
    common: {
      contactUs: 'Contact Us',
      learnMore: 'Learn More',
      readMore: 'Read More',
      viewMore: 'View More',
      close: 'Close',
      menu: 'Menu',
    },
  },

  ar: {
    nav: {
      home: 'الرئيسية',
      about: 'حول',
      whatWeDo: 'ما نفعله',
      sustainability: 'الاستدامة',
      careers: 'الوظائف',
      insights: 'الرؤى',
      contact: 'اتصل بنا',
      suppliers: 'الموردون',
    },
    footer: {
      offices: 'المكاتب',
      legal: 'قانوني',
      privacy: 'سياسة الخصوصية',
      paia: 'قانون الوصول إلى المعلومات',
      sitemap: 'خريطة الموقع',
      followUs: 'تابعنا',
      copyright: '© 2026 مينوبكس. جميع الحقوق محفوظة. شركة تابعة لمجموعة DRA العالمية.',
    },
    home: {
      tagline: 'متخصصون عالميون رائدون في عمليات المصانع والصيانة',
      heroHeading: 'التميز التشغيلي في التعدين العالمي',
      heroSubheading: 'تقديم حلول مبتكرة لعمليات التعدين عبر 14+ دول',
      learnMore: 'اعرف المزيد',
      surfaceOperations: 'العمليات السطحية',
      surfaceDesc: 'مصانع المعالجة والمختبرات وإدارة المياه',
      undergroundOperations: 'العمليات تحت الأرض',
      undergroundDesc: 'التعدين الآلي مع الجاهزية التشغيلية',
      seeWhatWeDo: 'شاهد ما نفعله',
      stats: {
        employees: 'الموظفون',
        countries: 'الدول',
        plants: 'المصانع المُشغّلة',
        ore: 'الخام/الشهر',
      },
      ourPeople: 'أكبر أصولنا هي أشخاصنا',
      ourPeopleDesc: 'مع أكثر من 2000 متخصص ماهر عبر 14+ دول، فريقنا هو أساس نجاحنا. نستثمر في السلامة والتطوير وخلق مكان عمل يزدهر فيه التميز.',
      partnership: 'جزء من مجموعة DRA العالمية',
      partnershipDesc: 'مينوبكس هي شركة رائدة ضمن مجموعة DRA العالمية، وتقدم حلول عمليات التعدين والصيانة على مستوى عالمي.',
      ctaDesc: 'هل أنت مستعد للشراكة مع مينوبكس؟ تواصل مع فريقنا اليوم.',
    },
    about: {
      heading: 'حول مينوبكس',
      subheading: 'تأسست عام 1996، أصبحت مينوبكس متخصصة عالمية رائدة في عمليات التعدين',
      history: 'تاريخنا',
      historyDesc: 'تأسست عام 1996 بالالتزام بالتميز التشغيلي، تطورت مينوبكس لتصبح شريكة موثوقة لعمليات التعدين في جميع أنحاء العالم. مع أكثر من 30 سنة من الخبرة المجمعة، نجحنا في تشغيل أكثر من 50 مصنعاً عبر سلع متعددة.',
      globalFootprint: 'الوجود العالمي',
      commodities: 'السلع التي نخدمها',
      coal: 'الفحم',
      chrome: 'الكروم',
      ironOre: 'خام الحديد',
      gold: 'الذهب',
      diamonds: 'الماس',
      pgm: 'معادن المجموعة البلاتينية',
      rareEarths: 'الأرض النادرة',
      lithium: 'الليثيوم',
      batteryMinerals: 'معادن البطاريات',
      criticalMinerals: 'المعادن الحرجة',
      values: 'قيمنا',
      focus: 'التركيز',
      focusDesc: 'الخبرة المركزة على التميز التشغيلي',
      accountability: 'المساءلة',
      accountabilityDesc: 'عمليات شفافة ومسؤولة',
      safety: 'السلامة',
      safetyDesc: 'عدم الإضرار بالأشخاص والبيئة',
      technology: 'التكنولوجيا',
      technologyDesc: 'الابتكار يدفع الكفاءة التشغيلية',
      integrity: 'النزاهة',
      integrityDesc: 'الممارسات الأخلاقية في جميع العمليات',
      respect: 'الاحترام',
      respectDesc: 'تقدير الناس والمجتمعات',
      ctaDesc: 'اكتشف ما نقوم به وكيف يمكننا مساعدة عمليات التعدين الخاصة بك.',
    },
    whatWeDo: {
      heading: 'ما نفعله',
      subheading: 'حلول شاملة لعمليات التعدين والصيانة',
      surfaceOpsHeading: 'العمليات السطحية',
      surfaceOpsDesc: 'من مصانع المعالجة إلى خدمات المختبرات، ندير دورة حياة العمليات السطحية الكاملة.',
      undergroundOpsHeading: 'العمليات تحت الأرض',
      undergroundOpsDesc: 'التعدين الآلي المتقدم مع نهج السلامة أولاً والجاهزية التشغيلية.',
      surface: {
        plants: 'مصانع ومرافق المعالجة',
        lab: 'خدمات المختبر وضمان الجودة',
        water: 'أنظمة إدارة المياه',
      },
      underground: {
        equipment: 'معدات تعدين آلية متقدمة',
        safety: 'بروتوكولات تشغيل تضع السلامة أولاً',
        readiness: 'الجاهزية التشغيلية والتحسين',
      },
      coreServices: 'خدماتنا الأساسية',
      services: {
        engineer: 'الهندسة',
        engineerDesc: 'تصميم وتحسين عمليات التعدين',
        optimise: 'التحسين',
        optimiseDesc: 'تعظيم الكفاءة والإنتاجية',
        innovate: 'الابتكار',
        innovateDesc: 'تطبيق التقنيات المتطورة',
        maintain: 'الصيانة',
        maintainDesc: 'ضمان عمليات موثوقة وآمنة',
      },
      qls: 'خدمات المختبر الجودة',
      qlsDesc: 'خدمات مختبرات حديثة لتحليل المعادن وضمان الجودة',
    },
    sustainability: {
      heading: 'الاستدامة والمسؤولية',
      subheading: 'التعدين بمسؤولية من أجل مستقبل مستدام',
      esd: 'الإدارة البيئية والتنمية',
      esdDesc: 'نحن ملتزمون بحماية النظم البيئية ودعم التنمية المجتمعية من خلال ممارسات التعدين المسؤولة.',
      csi: 'الاستثمار الاجتماعي المجتمعي',
      csiDesc: 'بناء علاقات قوية مع المجتمعات المحلية من خلال مبادرات التعليم والرعاية الصحية والتنمية الاقتصادية.',
      environmental: 'إدارة البيئة',
      water: 'إدارة المياه',
      waterDesc: 'أنظمة معالجة وإعادة تدوير متقدمة',
      land: 'استعادة الأراضي',
      landDesc: 'استعادة تدريجية لمواقع التعدين',
      air: 'التحكم في جودة الهواء',
      airDesc: 'أنظمة المراقبة والتحكم في الغبار',
      waste: 'إدارة النفايات',
      wasteDesc: 'إعادة تدوير والتخلص المسؤول',
      ourCommitment: 'التزامنا',
      commitmentDesc: 'نحن ملتزمون بممارسات التعدين المسؤولة التي تحمي البيئة وتدعم المجتمعات وتخلق قيمة دائمة للأجيال القادمة.',
      commitmentZeroHarm: 'لا ضرر للناس والبيئة',
      commitmentImpact: 'أثر صافٍ إيجابي على المجتمعات',
      commitmentTransparent: 'عمليات شفافة وخاضعة للمساءلة',
      ctaHeading: 'انضم إلينا في بناء مستقبل مستدام',
      ctaDesc: 'شارك مينوبكس لتحقيق أهداف التعدين الخاصة بك بشكل مسؤول.',
    },
    careers: {
      heading: 'الوظائف في مينوبكس',
      subheading: 'الناس هم أعظم أصولنا',
      whyJoin: 'لماذا تنضم إلى مينوبكس؟',
      whyJoinDesc: 'نحن نقدم تعويضات تنافسية وتطوير مهني ومكان عمل يقدر السلامة والنزاهة والابتكار.',
      whyChoose: 'لماذا تختار مينوبكس',
      applyNow: 'تقديم الطلب الآن',
      equalOpportunity: 'مينوبكس هي صاحب عمل توفر فرصاً متساوية. نحتفل بالتنوع والتزمنا بخلق بيئة شاملة.',
      equalOpportunityHeading: 'صاحب عمل متكافئ الفرص',
      ctaHeading: 'هل أنت مستعد للانضمام إلى فريقنا؟',
      ctaDesc: 'استكشف الفرص الحالية وابدأ مسيرتك المهنية مع مينوبكس.',
      benefits: {
        devTitle: 'التطوير المهني',
        devDesc: 'فرص التعلم المستمر والنمو المهني',
        cultureTitle: 'ثقافة تعاونية',
        cultureDesc: 'العمل مع محترفين موهوبين على مستوى عالمي',
        compTitle: 'تعويض تنافسي',
        compDesc: 'رواتب جذابة وحزم مزايا',
        globalTitle: 'فرص عالمية',
        globalDesc: 'العمل عبر 14+ دولة حول العالم',
      },
    },
    contact: {
      heading: 'اتصل بنا',
      subheading: 'تواصل مع فريقنا',
      offices: 'مكاتبنا',
      johannesburg: 'جوهانسبرج',
      lesotho: 'ليسوتو',
      mozambique: 'موزمبيق',
      saudiArabia: 'المملكة العربية السعودية',
      sendMessage: 'أرسل لنا رسالة',
      phone: 'هاتف',
      email: 'بريد إلكتروني',
      headquarters: 'المقر الرئيسي',
      form: {
        name: 'الاسم الكامل',
        email: 'عنوان البريد الإلكتروني',
        phone: 'رقم الهاتف',
        subject: 'الموضوع',
        message: 'الرسالة',
        submit: 'إرسال الرسالة',
        success: 'شكراً لرسالتك. سنعود إليك قريباً.',
      },
    },
    leadership: {
      heading: 'القيادة',
      subheading: 'تعرف على فريق قيادتنا',
      comingSoon: 'معلومات فريق القيادة قادمة قريباً.',
    },
    insights: {
      heading: 'الرؤى والمدونة',
      subheading: 'أحدث الأخبار والرؤى من مينوبكس',
      comingSoon: 'المدونة والرؤى قادمة قريباً.',
    },
    suppliers: {
      heading: 'الموردون',
      subheading: 'معلومات لمورديننا',
      comingSoon: 'معلومات وموارد الموردين قادمة قريباً.',
    },
    privacy: {
      heading: 'سياسة الخصوصية',
      intro: 'مينوبكس ملتزمة بحماية خصوصيتك. تشرح سياسة الخصوصية هذه كيفية جمعنا واستخدامنا وحماية معلوماتك الشخصية.',
      comingSoon: 'محتوى سياسة الخصوصية الكامل قادم قريباً.',
    },
    paia: {
      heading: 'قانون الوصول إلى المعلومات',
      subheading: 'قانون تشجيع الوصول إلى المعلومات',
      manualHeading: 'دليل قانون الوصول إلى المعلومات',
      intro: 'وفقاً لقانون تشجيع الوصول إلى المعلومات، تتيح مينوبكس الوصول إلى المعلومات التي تحتفظ بها الشركة.',
      comingSoon: 'دليل قانون الوصول إلى المعلومات وطلبات المعلومات قادمة قريباً.',
    },
    common: {
      contactUs: 'اتصل بنا',
      learnMore: 'اعرف المزيد',
      readMore: 'اقرأ المزيد',
      viewMore: 'عرض المزيد',
      close: 'إغلاق',
      menu: 'القائمة',
    },
  },

  el: {
    nav: {
      home: 'Αρχική',
      about: 'Σχετικά',
      whatWeDo: 'Τι κάνουμε',
      sustainability: 'Βιωσιμότητα',
      careers: 'Καριέρα',
      insights: 'Ενημερώσεις',
      contact: 'Επικοινωνία',
      suppliers: 'Προμηθευτές',
    },
    footer: {
      offices: 'Γραφεία',
      legal: 'Νομικά',
      privacy: 'Πολιτική Απορρήτου',
      paia: 'Νόμος Πρόσβασης σε Πληροφορίες',
      sitemap: 'Χάρτης Ιστότοπου',
      followUs: 'Ακολουθήστε μας',
      copyright: '© 2026 Minopex. Όλα τα δικαιώματα διατηρούνται. Εταιρεία του ομίλου DRA Global.',
    },
    home: {
      tagline: 'Κορυφαίοι παγκόσμιοι ειδικοί σε λειτουργίες εργοστασίων και συντήρηση',
      heroHeading: 'Λειτουργική Αριστεία στην Παγκόσμια Εξόρυξη',
      heroSubheading: 'Παροχή καινοτόμων λύσεων για λειτουργίες εξόρυξης σε 14+ χώρες',
      learnMore: 'Μάθετε περισσότερα',
      surfaceOperations: 'Επιφανειακές Λειτουργίες',
      surfaceDesc: 'Εργοστάσια επεξεργασίας, εργαστήρια και διαχείριση νερού',
      undergroundOperations: 'Υπόγειες Λειτουργίες',
      undergroundDesc: 'Μηχανοποιημένη εξόρυξη με λειτουργική ετοιμότητα',
      seeWhatWeDo: 'Δείτε τι κάνουμε',
      stats: {
        employees: 'Υπάλληλοι',
        countries: 'Χώρες',
        plants: 'Εργοστάσια σε Λειτουργία',
        ore: 'Μετάλλευμα/Μήνα',
      },
      ourPeople: 'Το μεγαλύτερο περιουσιακό μας στοιχείο είναι ο λαός μας',
      ourPeopleDesc: 'Με 2.000+ ειδικευμένους επαγγελματίες σε 14+ χώρες, η ομάδα μας είναι το θεμέλιο της επιτυχίας μας.',
      partnership: 'Μέρος του ομίλου DRA Global',
      partnershipDesc: 'Η Minopex είναι κορυφαία εταιρεία εντός του ομίλου DRA Global, παρέχοντας λύσεις λειτουργιών εξόρυξης και συντήρησης παγκόσμιας κλάσης.',
      ctaDesc: 'Έτοιμοι να συνεργαστείτε με τη Minopex; Επικοινωνήστε με την ομάδα μας σήμερα.',
    },
    about: {
      heading: 'Σχετικά με τη Minopex',
      subheading: 'Ιδρύθηκε το 1996, η Minopex έχει γίνει κορυφαία παγκόσμια ειδικός σε λειτουργίες εξόρυξης',
      history: 'Η Ιστορία μας',
      historyDesc: 'Ιδρύθηκε το 1996 με δέσμευση για λειτουργική αριστεία, η Minopex έχει εξελιχθεί σε αξιόπιστη συνεργάτιδα για λειτουργίες εξόρυξης παγκοσμίως.',
      globalFootprint: 'Παγκόσμια Παρουσία',
      commodities: 'Εμπορεύματα που Εξυπηρετούμε',
      coal: 'Άνθρακας',
      chrome: 'Χρώμιο',
      ironOre: 'Σίδηρος',
      gold: 'Χρυσός',
      diamonds: 'Διαμάντια',
      pgm: 'Μέταλλα Ομάδας Λευκοχρύσου',
      rareEarths: 'Σπάνια Γήινα Στοιχεία',
      lithium: 'Λίθιο',
      batteryMinerals: 'Μέταλλα Μπαταρίας',
      criticalMinerals: 'Κρίσιμα Μέταλλα',
      values: 'Οι Αξίες μας',
      focus: 'Εστίαση',
      focusDesc: 'Συγκεντρωμένη τεχνογνωσία στη λειτουργική αριστεία',
      accountability: 'Λογοδοσία',
      accountabilityDesc: 'Διαφανείς και υπεύθυνες λειτουργίες',
      safety: 'Ασφάλεια',
      safetyDesc: 'Μηδενική βλάβη για ανθρώπους και περιβάλλον',
      technology: 'Τεχνολογία',
      technologyDesc: 'Καινοτομία που οδηγεί την λειτουργική αποδοτικότητα',
      integrity: 'Ακεραιότητα',
      integrityDesc: 'Ηθικές πρακτικές σε όλες τις λειτουργίες',
      respect: 'Σεβασμός',
      respectDesc: 'Εκτίμηση ανθρώπων και κοινοτήτων',
      ctaDesc: 'Ανακαλύψτε τι κάνουμε και πώς μπορούμε να βοηθήσουμε τις λειτουργίες εξόρυξής σας.',
    },
    whatWeDo: {
      heading: 'Τι κάνουμε',
      subheading: 'Ολοκληρωμένες λύσεις λειτουργιών εξόρυξης και συντήρησης',
      surfaceOpsHeading: 'Επιφανειακές Λειτουργίες',
      surfaceOpsDesc: 'Από εργοστάσια επεξεργασίας έως υπηρεσίες εργαστηρίου, διαχειριζόμαστε τον πλήρη κύκλο ζωής επιφανειακών λειτουργιών.',
      undergroundOpsHeading: 'Υπόγειες Λειτουργίες',
      undergroundOpsDesc: 'Προηγμένη μηχανοποιημένη εξόρυξη με προσέγγιση ασφάλειας πρώτα και λειτουργική ετοιμότητα.',
      surface: {
        plants: 'Μονάδες επεξεργασίας & εγκαταστάσεις',
        lab: 'Εργαστηριακές υπηρεσίες & διασφάλιση ποιότητας',
        water: 'Συστήματα διαχείρισης νερού',
      },
      underground: {
        equipment: 'Προηγμένος εξοπλισμός μηχανοποιημένης εξόρυξης',
        safety: 'Πρωτόκολλα λειτουργίας με προτεραιότητα ασφάλειας',
        readiness: 'Λειτουργική ετοιμότητα & βελτιστοποίηση',
      },
      coreServices: 'Οι Βασικές μας Υπηρεσίες',
      services: {
        engineer: 'Μηχανική',
        engineerDesc: 'Σχεδίαση και βελτιστοποίηση λειτουργιών εξόρυξης',
        optimise: 'Βελτιστοποίηση',
        optimiseDesc: 'Μεγιστοποίηση αποδοτικότητας και παραγωγικότητας',
        innovate: 'Καινοτομία',
        innovateDesc: 'Εφαρμογή τεχνολογιών αιχμής',
        maintain: 'Συντήρηση',
        maintainDesc: 'Διασφάλιση αξιόπιστων και ασφαλών λειτουργιών',
      },
      qls: 'Υπηρεσίες Εργαστηρίου Ποιότητας',
      qlsDesc: 'Υπηρεσίες εργαστηρίου τελευταίας τεχνολογίας για ανάλυση μεταλλευμάτων και διασφάλιση ποιότητας',
    },
    sustainability: {
      heading: 'Βιωσιμότητα και Ευθύνη',
      subheading: 'Εξόρυξη με ευθύνη για ένα βιώσιμο μέλλον',
      esd: 'Περιβαλλοντική Διαχείριση και Ανάπτυξη',
      esdDesc: 'Είμαστε δεσμευμένοι στην προστασία των οικοσυστημάτων και την υποστήριξη της κοινοτικής ανάπτυξης μέσω υπεύθυνων πρακτικών εξόρυξης.',
      csi: 'Κοινοτική Κοινωνική Επένδυση',
      csiDesc: 'Δημιουργία ισχυρών σχέσεων με τοπικές κοινότητες μέσω πρωτοβουλιών εκπαίδευσης, υγειονομικής περίθαλψης και οικονομικής ανάπτυξης.',
      environmental: 'Περιβαλλοντική Διαχείριση',
      water: 'Διαχείριση Νερού',
      waterDesc: 'Προηγμένα συστήματα επεξεργασίας και ανακύκλωσης',
      land: 'Αποκατάσταση Εδάφους',
      landDesc: 'Σταδιακή αποκατάσταση χώρων εξόρυξης',
      air: 'Έλεγχος Ποιότητας Αέρα',
      airDesc: 'Συστήματα παρακολούθησης και ελέγχου σκόνης',
      waste: 'Διαχείριση Αποβλήτων',
      wasteDesc: 'Υπεύθυνη ανακύκλωση και διάθεση',
      ourCommitment: 'Η Δέσμευσή μας',
      commitmentDesc: 'Είμαστε δεσμευμένοι σε υπεύθυνες πρακτικές εξόρυξης που προστατεύουν το περιβάλλον, υποστηρίζουν τις κοινότητες και δημιουργούν διαρκή αξία για τις μελλοντικές γενιές.',
      commitmentZeroHarm: 'Μηδενική βλάβη για ανθρώπους και περιβάλλον',
      commitmentImpact: 'Καθαρή θετική επίδραση στις κοινότητες',
      commitmentTransparent: 'Διαφανείς και υπόλογες λειτουργίες',
      ctaHeading: 'Ελάτε Μαζί μας για ένα Βιώσιμο Μέλλον',
      ctaDesc: 'Συνεργαστείτε με τη Minopex για να πετύχετε τους στόχους εξόρυξής σας υπεύθυνα.',
    },
    careers: {
      heading: 'Καριέρα στη Minopex',
      subheading: 'Οι άνθρωποι είναι το μεγαλύτερο περιουσιακό μας στοιχείο',
      whyJoin: 'Γιατί να Ενταχθείτε στη Minopex;',
      whyJoinDesc: 'Προσφέρουμε ανταγωνιστική αποζημίωση, επαγγελματική ανάπτυξη και πολιτισμό χώρου εργασίας που εκτιμά την ασφάλεια, την ακεραιότητα και την καινοτομία.',
      whyChoose: 'Γιατί να Επιλέξετε τη Minopex',
      applyNow: 'Υποβάλετε Αίτηση Τώρα',
      equalOpportunity: 'Η Minopex είναι Εργοδότης Ίσων Ευκαιριών. Γιορτάζουμε την ποικιλομορφία και είμαστε δεσμευμένοι στη δημιουργία ενός χώρου που περιλαμβάνει όλους.',
      equalOpportunityHeading: 'Εργοδότης Ίσων Ευκαιριών',
      ctaHeading: 'Έτοιμοι να Ενταχθείτε στην Ομάδα μας;',
      ctaDesc: 'Εξερευνήστε τρέχουσες ευκαιρίες και ξεκινήστε την καριέρα σας με τη Minopex.',
      benefits: {
        devTitle: 'Επαγγελματική Ανάπτυξη',
        devDesc: 'Συνεχής μάθηση και ευκαιρίες καριέρας',
        cultureTitle: 'Συνεργατική Κουλτούρα',
        cultureDesc: 'Εργαστείτε με ταλαντούχους επαγγελματίες παγκοσμίως',
        compTitle: 'Ανταγωνιστική Αποζημίωση',
        compDesc: 'Ελκυστικοί μισθοί και παροχές',
        globalTitle: 'Παγκόσμιες Ευκαιρίες',
        globalDesc: 'Εργαστείτε σε 14+ χώρες παγκοσμίως',
      },
    },
    contact: {
      heading: 'Επικοινωνία',
      subheading: 'Επικοινωνήστε με την ομάδα μας',
      offices: 'Τα Γραφεία μας',
      johannesburg: 'Johannesburg',
      lesotho: 'Lesotho',
      mozambique: 'Μοζαμβίκη',
      saudiArabia: 'Σαουδική Αραβία',
      sendMessage: 'Στείλτε μας Μήνυμα',
      phone: 'Τηλέφωνο',
      email: 'Email',
      headquarters: 'Κεντρικά Γραφεία',
      form: {
        name: 'Πλήρες Όνομα',
        email: 'Διεύθυνση Email',
        phone: 'Αριθμός Τηλεφώνου',
        subject: 'Θέμα',
        message: 'Μήνυμα',
        submit: 'Αποστολή Μηνύματος',
        success: 'Ευχαριστούμε για το μήνυμά σας. Θα επικοινωνήσουμε σύντομα.',
      },
    },
    leadership: {
      heading: 'Ηγεσία',
      subheading: 'Γνωρίστε την ομάδα ηγεσίας μας',
      comingSoon: 'Πληροφορίες για την ομάδα ηγεσίας σύντομα.',
    },
    insights: {
      heading: 'Αναλύσεις & Blog',
      subheading: 'Τελευταία νέα και αναλύσεις από τη Minopex',
      comingSoon: 'Blog και αναλύσεις σύντομα.',
    },
    suppliers: {
      heading: 'Προμηθευτές',
      subheading: 'Πληροφορίες για τους προμηθευτές μας',
      comingSoon: 'Πληροφορίες και πόροι για προμηθευτές σύντομα.',
    },
    privacy: {
      heading: 'Πολιτική Απορρήτου',
      intro: 'Η Minopex δεσμεύεται να προστατεύει την ιδιωτικότητά σας. Αυτή η πολιτική απορρήτου εξηγεί πώς συλλέγουμε, χρησιμοποιούμε και προστατεύουμε τα προσωπικά σας στοιχεία.',
      comingSoon: 'Πλήρες περιεχόμενο πολιτικής απορρήτου σύντομα.',
    },
    paia: {
      heading: 'Νόμος PAIA',
      subheading: 'Νόμος για την Προώθηση Πρόσβασης σε Πληροφορίες',
      manualHeading: 'Εγχειρίδιο PAIA',
      intro: 'Σύμφωνα με τον Νόμο για την Προώθηση Πρόσβασης σε Πληροφορίες, η Minopex παρέχει πρόσβαση σε πληροφορίες που τηρεί η εταιρεία.',
      comingSoon: 'Εγχειρίδιο PAIA και αιτήματα πληροφοριών σύντομα.',
    },
    common: {
      contactUs: 'Επικοινωνία',
      learnMore: 'Μάθετε περισσότερα',
      readMore: 'Διαβάστε περισσότερα',
      viewMore: 'Προβολή Περισσότερων',
      close: 'Κλείσιμο',
      menu: 'Μενού',
    },
  },

  es: {
    nav: {
      home: 'Inicio',
      about: 'Sobre nosotros',
      whatWeDo: 'Qué hacemos',
      sustainability: 'Sostenibilidad',
      careers: 'Empleo',
      insights: 'Perspectivas',
      contact: 'Contacto',
      suppliers: 'Proveedores',
    },
    footer: {
      offices: 'Oficinas',
      legal: 'Legal',
      privacy: 'Política de privacidad',
      paia: 'PAIA',
      sitemap: 'Mapa del sitio',
      followUs: 'Síguenos',
      copyright: '© 2026 Minopex. Todos los derechos reservados. Una empresa de DRA Global.',
    },
    home: {
      tagline: 'Especialistas líderes mundiales en operaciones de plantas y mantenimiento',
      heroHeading: 'Excelencia Operacional en Minería Global',
      heroSubheading: 'Soluciones innovadoras para operaciones mineras en más de 14 países',
      learnMore: 'Saber más',
      surfaceOperations: 'Operaciones de Superficie',
      surfaceDesc: 'Plantas de procesamiento, laboratorios y gestión del agua',
      undergroundOperations: 'Operaciones Subterráneas',
      undergroundDesc: 'Minería mecanizada con preparación operacional',
      seeWhatWeDo: 'Ver qué hacemos',
      stats: {
        employees: 'Empleados',
        countries: 'Países',
        plants: 'Plantas Operadas',
        ore: 'Mineral/Mes',
      },
      ourPeople: 'Nuestro Mayor Activo es Nuestra Gente',
      ourPeopleDesc: 'Con más de 2.000 profesionales cualificados en más de 14 países, nuestro equipo es el pilar de nuestro éxito. Invertimos en seguridad, desarrollo y en crear un entorno donde la excelencia florece.',
      partnership: 'Parte de DRA Global',
      partnershipDesc: 'Minopex es una empresa líder dentro del grupo DRA Global, que ofrece soluciones de operaciones mineras y mantenimiento de clase mundial.',
      ctaDesc: '¿Listo para asociarse con Minopex? Póngase en contacto con nuestro equipo hoy.',
    },
    about: {
      heading: 'Sobre Minopex',
      subheading: 'Fundada en 1996, Minopex se ha convertido en una especialista global líder en operaciones mineras',
      history: 'Nuestra Historia',
      historyDesc: 'Fundada en 1996 con un compromiso con la excelencia operacional, Minopex ha evolucionado hasta convertirse en un socio de confianza para operaciones mineras en todo el mundo. Con más de 30 años de experiencia combinada, hemos operado con éxito más de 50 plantas en múltiples materias primas.',
      globalFootprint: 'Presencia Global',
      commodities: 'Materias Primas que Atendemos',
      coal: 'Carbón',
      chrome: 'Cromo',
      ironOre: 'Mineral de Hierro',
      gold: 'Oro',
      diamonds: 'Diamantes',
      pgm: 'Metales del Grupo del Platino',
      rareEarths: 'Tierras Raras',
      lithium: 'Litio',
      batteryMinerals: 'Minerales de Batería',
      criticalMinerals: 'Minerales Críticos',
      values: 'Nuestros Valores',
      focus: 'Enfoque',
      focusDesc: 'Experiencia concentrada en la excelencia operacional',
      accountability: 'Responsabilidad',
      accountabilityDesc: 'Operaciones transparentes y responsables',
      safety: 'Seguridad',
      safetyDesc: 'Cero daño a personas y al medio ambiente',
      technology: 'Tecnología',
      technologyDesc: 'Innovación que impulsa la eficiencia operacional',
      integrity: 'Integridad',
      integrityDesc: 'Prácticas éticas en todas las operaciones',
      respect: 'Respeto',
      respectDesc: 'Valorar a las personas y comunidades',
      ctaDesc: 'Descubra qué hacemos y cómo podemos ayudar a sus operaciones mineras.',
    },
    whatWeDo: {
      heading: 'Qué Hacemos',
      subheading: 'Soluciones integrales de operaciones mineras y mantenimiento',
      surfaceOpsHeading: 'Operaciones de Superficie',
      surfaceOpsDesc: 'Desde plantas de procesamiento hasta servicios de laboratorio, gestionamos el ciclo de vida completo de las operaciones de superficie.',
      undergroundOpsHeading: 'Operaciones Subterráneas',
      undergroundOpsDesc: 'Minería mecanizada avanzada con enfoque de seguridad primero y preparación operacional.',
      surface: {
        plants: 'Plantas e instalaciones de procesamiento',
        lab: 'Servicios de laboratorio y garantía de calidad',
        water: 'Sistemas de gestión del agua',
      },
      underground: {
        equipment: 'Equipos avanzados de minería mecanizada',
        safety: 'Protocolos operacionales con seguridad primero',
        readiness: 'Preparación operacional y optimización',
      },
      coreServices: 'Nuestros Servicios Principales',
      services: {
        engineer: 'Ingeniería',
        engineerDesc: 'Diseño y optimización de operaciones mineras',
        optimise: 'Optimizar',
        optimiseDesc: 'Maximizar la eficiencia y la productividad',
        innovate: 'Innovar',
        innovateDesc: 'Implementar tecnologías de vanguardia',
        maintain: 'Mantenimiento',
        maintainDesc: 'Garantizar operaciones confiables y seguras',
      },
      qls: 'Servicios de Laboratorio de Calidad',
      qlsDesc: 'Servicios de laboratorio de última generación para análisis de minerales y garantía de calidad',
    },
    sustainability: {
      heading: 'Sostenibilidad y Responsabilidad',
      subheading: 'Minería responsable para un futuro sostenible',
      esd: 'Gestión Ambiental y Desarrollo',
      esdDesc: 'Estamos comprometidos a proteger los ecosistemas y apoyar el desarrollo comunitario a través de prácticas mineras responsables.',
      csi: 'Inversión Social Comunitaria',
      csiDesc: 'Construyendo relaciones sólidas con las comunidades locales a través de iniciativas de educación, salud y desarrollo económico.',
      environmental: 'Gestión Ambiental',
      water: 'Gestión del Agua',
      waterDesc: 'Sistemas avanzados de tratamiento y reciclaje',
      land: 'Rehabilitación de Terrenos',
      landDesc: 'Restauración progresiva de sitios mineros',
      air: 'Control de Calidad del Aire',
      airDesc: 'Sistemas de monitoreo y control del polvo',
      waste: 'Gestión de Residuos',
      wasteDesc: 'Reciclaje y eliminación responsable',
      ourCommitment: 'Nuestro Compromiso',
      commitmentDesc: 'Estamos comprometidos con prácticas mineras responsables que protejan el medio ambiente, apoyen a las comunidades y creen valor duradero para las generaciones futuras.',
      commitmentZeroHarm: 'Cero daño a personas y al medio ambiente',
      commitmentImpact: 'Impacto neto positivo en las comunidades',
      commitmentTransparent: 'Operaciones transparentes y responsables',
      ctaHeading: 'Únase a Nosotros para Construir un Futuro Sostenible',
      ctaDesc: 'Asóciese con Minopex para alcanzar sus objetivos mineros de forma responsable.',
    },
    careers: {
      heading: 'Empleo en Minopex',
      subheading: 'Las personas son nuestro mayor activo',
      whyJoin: '¿Por qué unirse a Minopex?',
      whyJoinDesc: 'Ofrecemos compensación competitiva, desarrollo profesional y una cultura laboral que valora la seguridad, la integridad y la innovación.',
      whyChoose: 'Por qué elegir Minopex',
      applyNow: 'Solicitar ahora',
      equalOpportunity: 'Minopex es un Empleador de Igualdad de Oportunidades. Celebramos la diversidad y estamos comprometidos a crear un entorno inclusivo.',
      equalOpportunityHeading: 'Empleador de Igualdad de Oportunidades',
      ctaHeading: '¿Listo para unirse a nuestro equipo?',
      ctaDesc: 'Explore las oportunidades actuales y comience su carrera con Minopex.',
      benefits: {
        devTitle: 'Desarrollo Profesional',
        devDesc: 'Aprendizaje continuo y oportunidades de crecimiento profesional',
        cultureTitle: 'Cultura Colaborativa',
        cultureDesc: 'Trabaje con profesionales talentosos a nivel mundial',
        compTitle: 'Compensación Competitiva',
        compDesc: 'Salarios atractivos y paquetes de beneficios',
        globalTitle: 'Oportunidades Globales',
        globalDesc: 'Trabaje en más de 14 países en todo el mundo',
      },
    },
    contact: {
      heading: 'Contáctenos',
      subheading: 'Póngase en contacto con nuestro equipo',
      offices: 'Nuestras Oficinas',
      johannesburg: 'Johannesburgo',
      lesotho: 'Lesoto',
      mozambique: 'Mozambique',
      saudiArabia: 'Arabia Saudita',
      sendMessage: 'Envíenos un Mensaje',
      phone: 'Teléfono',
      email: 'Correo electrónico',
      headquarters: 'Sede Central',
      form: {
        name: 'Nombre Completo',
        email: 'Dirección de Correo Electrónico',
        phone: 'Número de Teléfono',
        subject: 'Asunto',
        message: 'Mensaje',
        submit: 'Enviar Mensaje',
        success: 'Gracias por su mensaje. Nos pondremos en contacto pronto.',
      },
    },
    leadership: {
      heading: 'Liderazgo',
      subheading: 'Conozca a nuestro equipo directivo',
      comingSoon: 'Información del equipo directivo próximamente.',
    },
    insights: {
      heading: 'Perspectivas y Blog',
      subheading: 'Últimas noticias y perspectivas de Minopex',
      comingSoon: 'Blog y perspectivas próximamente.',
    },
    suppliers: {
      heading: 'Proveedores',
      subheading: 'Información para nuestros proveedores',
      comingSoon: 'Información y recursos para proveedores próximamente.',
    },
    privacy: {
      heading: 'Política de Privacidad',
      intro: 'Minopex se compromete a proteger su privacidad. Esta política de privacidad explica cómo recopilamos, usamos y protegemos su información personal.',
      comingSoon: 'Contenido completo de la política de privacidad próximamente.',
    },
    paia: {
      heading: 'PAIA',
      subheading: 'Ley de Promoción del Acceso a la Información',
      manualHeading: 'Manual PAIA',
      intro: 'De conformidad con la Ley de Promoción del Acceso a la Información, Minopex proporciona acceso a la información en poder de la empresa.',
      comingSoon: 'Manual PAIA y solicitudes de información próximamente.',
    },
    common: {
      contactUs: 'Contáctenos',
      learnMore: 'Saber más',
      readMore: 'Leer más',
      viewMore: 'Ver más',
      close: 'Cerrar',
      menu: 'Menú',
    },
  },
};

export function getTranslation(lang: Language, path: string): string {
  const keys = path.split('.');
  let value: any = translations[lang];

  for (const key of keys) {
    if (value && typeof value === 'object' && key in value) {
      value = value[key];
    } else {
      // Fallback to English
      let fallback: any = translations['en'];
      for (const k of keys) {
        if (fallback && typeof fallback === 'object' && k in fallback) {
          fallback = fallback[k];
        } else {
          return path;
        }
      }
      return typeof fallback === 'string' ? fallback : path;
    }
  }

  return typeof value === 'string' ? value : path;
}
